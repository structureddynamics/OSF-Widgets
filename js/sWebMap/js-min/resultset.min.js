...
this.getSubjectsByType=resultset_getSubjectsByType;this.getSubject=resultset_getSubject}Resultset.prototype.getSubjectByURI=function(b){for(var a=0;a<this.subjects.length;a++)if(b==this.subjects[a].uri)return this.subjects[a]};function subject_getPredicateValues(b){var a=[];if("predicate"in this)for(var c=0;c<this.predicate.length;c++){var d=this.predicate[c],e;for(e in d)(this.prefixize(b)==e||b==e)&&a.push(d[e])}return a}
...
"object"==typeof this.subjects[b].predicate[a][c]&&"uri"in this.subjects[b].predicate[a][c]?this.isPrefixed(this.prefixize(c))?n3+("  "+this.prefixize(c)+" <"+this.subjects[b].predicate[a][c].uri+"> ;\n"):n3+("  <"+c+"> <"+this.subjects[b].predicate[a][c].uri+"> ;\n"):this.isPrefixed(this.prefixize(c))?n3+("  "+this.prefixize(c)+' "'+escapeN3(this.subjects[b].predicate[a][c])+'" ;\n'):n3+("  <"+c+'> "'+escapeN3(this.subjects[b].predicate[a][c])+'" ;\n');n3=n3.substr(0,n3.length-2)+". \n"}return n3}
function escapeN3(b){return b.replace('"','\\"')}function subject_addAttributeValue(b,a){if("predicate"in this){var c={};c[this.prefixize(b)]=a;this.predicate.push(c)}}
function subject_rename(b){if("predicate"in this)for(var a=0;a<this.predicate.length;a++){var c=this.predicate[a],d;for(d in c)for(var e=0;e<prefLabelAttributes.length;e++)if(prefLabelAttributes[e]==this.unprefixize(d)||prefLabelAttributes[e]==d){c[d]=b;return}}this.addAttributeValue("http://www.w3.org/2004/02/skos/core#prefLabel",b)}
