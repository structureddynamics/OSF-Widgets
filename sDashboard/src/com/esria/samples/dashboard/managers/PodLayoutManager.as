...
import com.sd.semantic.components.sDashboard;
...
public class PodLayoutManager extends EventDispatcher
{
	...
	private static const TASKBAR_HEIGHT:Number = 40;		// The height of the area for minimized pods.
  ...
  public function resizeCanvas(e:ResizeEvent):void
  {
    updateLayout(false);  
  }  
	...
	public function updateLayout(tween:Boolean=true, dashboard:sDashboard=null):void
	{
		var len:Number = items.length;
    ...
    if((itemWidth <= 0 || itemHeight <= 0) && dashboard != null)
    {
      itemWidth = Math.round(dashboard.viewStack.width / numCols - ((POD_GAP * (numCols - 1)) / numCols));
      itemHeight = Math.round(dashboard.viewStack.height / numRows - ((POD_GAP * (numRows - 1)) / numRows));      
    }
