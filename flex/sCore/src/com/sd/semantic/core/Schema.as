package com.sd.semantic.core
{
  ...
  public class Schema extends EventDispatcher
  {
    ...
    */
    ...
    private var schemaFileName:String = "";
    ...
    {
      schemaFileName = schemaFile.substr(schemaFile.lastIndexOf("/") + 1).replace(".xml", "");
      
      dispatchEvent(new SemanticComponentLoadEvent("start", {
        label: "Downloading " + schemaFileName + " schema...",
        progress: 5
      }, true, false));
      
      var xmlLoader:URLLoader = new URLLoader();
      
      xmlLoader.addEventListener(Event.COMPLETE, processXmlSchemaCompleteHandler);
      xmlLoader.addEventListener(ProgressEvent.PROGRESS, processXmlSchemaProgressHandler);
    ...
    /**
     * Get all sub-attributes of a given attribute ID.
     *  
     * @param attributeID Prefixed ID of the type to get (example: foaf_name).
     * @param recursively If this parameter is true, it will recursively browse the type structure
     *        to get all sub-atttributes of this attribute. Otherwise, it will only get the immediate sub-attributes.
     * @return An array of all the sub-attributes of the attribute type ID. 
     */
    ...
    /**
     * Get all attributes that shares the same unitType
     *  
     * @param attribute URI of the attribute. Can in its full URI form like "http://foo.com/bar/..." or its internal 
     *                  abbreged form such as "foo_Bar...".
     * @param sameUnitTypeAttributes All the attributes that share the same unit type
     * 
     * @return Returns nothing
     */
    public function getSameUnitTypeAttributes(attribute:String, sameUnitTypeAttributes:Array):void
    {
      /** Schema attribute in the schema */
      var targetAttribute:SchemaAttribute = getAttribute(attribute);      

      for each(var attr:SchemaAttribute in this.attributes)
      {
        if(attr.unitType == targetAttribute.unitType)
        {
          sameUnitTypeAttributes.push(attr);
        }
      }
    }    

    private function processXmlSchemaProgressHandler(event:ProgressEvent):void
    {
      dispatchEvent(new SemanticComponentLoadEvent("start", {
        label: "Downloading " + schemaFileName + " schema...",
        progress: 5
     ...
     */
    private function processXmlSchemaCompleteHandler(event:Event):void
    {
      dispatchEvent(new SemanticComponentLoadEvent("start", {
        label: "Loading " + schemaFileName + " schema...",
        progress: 5
      }, true, false));   
      ...
      {
        Alert.show("Error loading the "+schemaFileName+" Schema");
        isError = true;
      }
      ...
      */
      ...
      this.removeEventListener(Event.COMPLETE, processXmlSchemaCompleteHandler);
    }
    ...
    private var stopIniniteLoop:Array = [];
    ...
    private function processSchema(schemaXML:XML):void
    {
      ...
      dispatchEvent(new SemanticComponentLoadEvent("start", {
        label: schemaFileName + " schema loaded...",
        progress: 5
      }, true, false));         
    }
    ...
    {
      if(stopIniniteLoop.indexOf(type.uri) != -1)
      {
        return;
      }
      else
      {      
        stopIniniteLoop.push(type.uri);
      }
