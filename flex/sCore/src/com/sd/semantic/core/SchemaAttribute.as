package com.sd.semantic.core
{
  ...
  public class SchemaAttribute
  {
    ...
    */
    ...
    /**
     * URI reference to a unit type ontology. The sco:unitType property is used to determine the 
     * type of unit referenced by a property. For example, if a data property has xsd:float as 
     * range, then sco:unitType determine what kind of things this number refers to.
     * 
     * The semantic components will make all the properties that share the same sco:unitType 
     * comparable (so, possibly displayable on the same semantic component, such as the 
     * sBarChart and the sLinearChart). 
     * 
    ...
    public function SchemaAttribute(attribute:XML, ns:Namespaces = null)
    {
      ...
      /** Parse unit type attributes */
      if(attribute.hasOwnProperty("unitType"))
      {
      ...
      {
        displayControls.push(namespaces.getVariable(displayControl.toString().replace("sco_", "").replace("http://purl.org/ontology/sco#", "")));
      }
